...
Adaptation note: Every adopted pattern in this document (PRESERVE/IMPROVE revision, best-version tracking, completeness rubric weight ≥20%, agent persona docs, reasoning-before-scoring, min-2-iterations, feedback-cleared-after-one, style anchor, diminishing-returns auto-escalation) applies to CreatorOS without modification. eLearn examples (course modules, video scenes) illustrate the patterns — substitute creator artifacts (linkedin_post, long_form_article, LongFormMaster sections) and the same rules govern.
───────────────────────────────────────────────────────────────────── -->
