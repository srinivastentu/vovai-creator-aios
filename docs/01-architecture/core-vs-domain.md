...
Adaptation note: eLearn-specific examples (K-12 video, Bloom) remain in the original sections for reference. They illustrate the same patterns CreatorOS uses with creator-domain entities (persona, niche, linkedin_post, hook).
───────────────────────────────────────────────────────────────────── -->
...
| Component | Three-question test | Location |
|-----------|-------------------|----------|
| 6 ReviewAction types | No change for Film, no domain words, reusable as-is | core/review/types.ts |
